...
void flip_y(unsigned char *img, int w, int h, int c) {
    for (int y = 0; y < h / 2; y++) {
...
unsigned char *to_grayscale(unsigned char *img, int w, int h, int *c) {
    if (*c < 3) return img;
...
void limit_colors(unsigned char *img, int w, int h, int c, int total_colors) {
